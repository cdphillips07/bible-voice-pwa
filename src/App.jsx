      ...
      {[0.6, 1, 0.7, 1.2, 0.5, 0.9, 0.6].map((h, i) => (
        <div
          ...
          style={{
            ...
            animation: active
              ? `wvpulse${i % 3} 0.6s ease-in-out infinite alternate`
              : "none",
...
export default function App() {
  ...
  // ── Core ask flow: calls Netlify Functions ──
  const askBible = async (q) => {
    ...
    try {
      // 1️⃣ Ask Claude via Netlify Function
      ...
      if (!askRes.ok) {
        throw new Error(`Scripture fetch failed (${askRes.status})`);
      }
      ...
      // 2️⃣ Convert to speech via Netlify Function
      ...
      if (!speakRes.ok) {
        throw new Error(`Voice synthesis failed (${speakRes.status})`);
      }
      ...
      <div style={{
        ...
        {inputMode === "text" && (
          <>
            ...
            >
              {loading      ? "Seeking Scripture…"
               : audioLoading ? "Preparing Voice…"
               : "Seek the Word"}
            </button>
          </>
        )}
              ...
              {isRecording && (
                <>
                  ...
                  <div style={{
                    ...
                    borderRadius: "50%",
                    border: "1px solid rgba(201,168,76,0.2)",
                    animation: "ripple 1s ease-out infinite 0.3s",
                  }} />
                </>
              )}
              <button
